...
module.exports.postTourCollectore = async (req, res) => {
	try {
		const result = await tourService.postTourService(req.body);
		res.status(200).json({
			status: 'success',
...
module.exports.getAllTourCollectore = async (req, res) => {
	try {
		...
		const result = await tourService.getAllTourService(query);
		res.status(200).json({
			status: 'Success',
			data: result,
		});
...
module.exports.getATourByIdCollectore = async (req, res) => {
	try {
		...
		if (mongoose.Types.ObjectId.isValid(id)) {
			const result = await tourService.getATourByIdService(id);
